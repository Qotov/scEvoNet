...
def draw_network(
    ...
    *,
    ...
    ----------
    gene
        Same as :func:`scevonet.pl.draw_net`: iterable of nodes to label, ``str`` for one node,
        or ``None`` / ``False`` for automatic labeling of salient nodes (degree or betweenness).
    layout
    ...
    highlight = ({gene} if isinstance(gene, str) else set(gene)) if use_highlight else None
