...
from scevonet.pl._basic import finish_matplotlib_figure
...
class Sample:
    ...
    def _generate_ys(self) -> None:
        ...
        for cell_type in sorted(set(self.cell_types)):
            data[cell_type] = [1 if ct == cell_type else 0 for ct in self.cell_types]
        self.cell_types_df = pd.DataFrame(data, index=self.matrix.index)
